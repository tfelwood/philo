...
/*   Updated: 2022/06/17 13:53:32 by tfelwood         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_eat(t_philo *ph)
{
	...
	ft_print(ph->id, HAS_TAKEN_A_FORK, ph->info);
	if (ph->info->num == 1)
	{
...
void *ft_philo_live(void *data)
{
	...
	if (ph->id % 2 == 0)
		usleep(200);
	ft_print(ph->id, THINK, ph->info);
	while (1)
	{
		if (ft_eat(ph))
			break ;
		ft_print(ph->id, SLEEP, ph->info);
		ft_sleep(ph->info->sleep);
...
int	ft_philo_arr_init(t_philo **arr, t_info *info)
{
	...
	*arr = (t_philo *)malloc(info->num * sizeof(t_philo));
	...
	while (i < info->num)
	{
		if (ft_philo_init(i + 1, *arr + i, info))
		{
